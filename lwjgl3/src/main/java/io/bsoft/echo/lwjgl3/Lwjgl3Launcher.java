...
public class Lwjgl3Launcher {
    public static void main(String[] args) {
        ...
        if (!displayAvailable()) {
            System.err.println("ECHO: no active display found (is the screen asleep or is this a headless session?). "
                + "Cannot open a window.");
            System.exit(2);
        }

        new Lwjgl3Application(new EchoGame(EchoGame.LaunchOptions.parse(args)), getDefaultConfiguration());
    }

    private static Lwjgl3ApplicationConfiguration getDefaultConfiguration() {
        Lwjgl3ApplicationConfiguration configuration = new Lwjgl3ApplicationConfiguration();
        configuration.setTitle(io.bsoft.echo.GameConfig.TITLE);
        ...
        configuration.setWindowedMode(io.bsoft.echo.GameConfig.REFERENCE_WIDTH, io.bsoft.echo.GameConfig.REFERENCE_HEIGHT);
...
//        configuration.setOpenGLEmulation(Lwjgl3ApplicationConfiguration.GLEmulation.ANGLE_GLES20, 0, 0);
    ...
    /** GLFW ignores sleeping displays; LibGDX would otherwise fail with an opaque NullPointerException. */
    private static boolean displayAvailable() {
        ...
        return monitors != null && monitors.limit() > 0;
    }
}
